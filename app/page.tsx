      ...
      <main className="text-bone">
        ...
        <section className="relative overflow-hidden px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
          <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] lg:items-center">
            <div>
              ...
              <h1 className="mt-5 max-w-4xl font-display text-5xl uppercase leading-none tracking-normal text-bone sm:text-6xl lg:text-7xl">
            ...
            <div className="rounded border border-white/10 bg-panel/76 p-4 shadow-violet backdrop-blur-xl">
              <div className="rounded border border-white/10 bg-ink/80 p-5">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-goldSoft">
                  Content pack preview
                </p>
                <div className="mt-4 grid gap-3">
                  {["LinkedIn post", "Instagram caption", "TikTok script", "X thread", "Carousel outline", "Email draft"].map((item) => (
                    <div key={item} className="rounded border border-white/10 bg-white/[0.04] p-3">
        ...
        <section className="border-y border-white/10 bg-white/[0.025] px-4 py-14 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-7xl">
            <h2 className="max-w-3xl font-display text-3xl uppercase tracking-normal text-bone sm:text-4xl">
        ...
        <section id="features" className="px-4 py-14 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-7xl">
            <h2 className="font-display text-3xl uppercase tracking-normal text-bone sm:text-4xl">
              A complete content workflow
            </h2>
            <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
              {features.map((feature) => (
                <div key={feature} className="rounded border border-white/10 bg-white/[0.035] p-4 text-sm font-semibold text-bone">
                  {feature}
                </div>
              ))}
            </div>
          ...
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mt-8 grid gap-4 lg:grid-cols-3">
              {pricing.map((plan) => (
                <article
                  key={plan.name}
                  className={`rounded border p-5 ${plan.featured ? "border-gold/70 bg-gold/10 shadow-gold" : "border-white/10 bg-coal/70"}`}
                >
                  <BrandLogo showWordmark={false} size="sm" />
                  <h3 className="mt-4 text-xl font-semibold text-bone">{plan.name}</h3>
                  <p className="mt-2 font-display text-3xl uppercase text-goldSoft">{plan.price}</p>
                  ...
                  <a
                    href="#workspace"
                    className="mt-6 flex min-h-11 items-center justify-center rounded border border-violet/70 bg-violet px-4 text-sm font-semibold text-white transition hover:bg-violetDeep"
                  >
                    Start creating
                  </a>
                </article>
